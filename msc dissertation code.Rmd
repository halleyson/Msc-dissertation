...
Because the original data was not encoded in UFT-8, so after download the original data, has to open it with excel and re-save it with UFT-8 encoded .csv file. But if knit/publish is not required, then this process is not required, but for loading the data (using readr), it has to be read_csv but not read.csv. 
...
###(06/08/21 update: realize I could do it after change it from wide to long, then only the column: "measurement" needs to perform the change.)
...
###Ordinal logistic regression (best to apply when using likert scale)
```{r, message=FALSE}
...
ggplot(ggpredict_ordinaldata, aes(x = Rating, y = predicted)) + 
  ...
  theme_minimal() + facet_wrap(~group) + xlab("Likelihood of Sharing") + ylab("Predicted Probability of Sharing") + 
  geom_label_repel(aes(label=sprintf("%.3f", predicted),fill = Headlineveracity), box.padding = 0.5, min.segment.length = unit(0, 'lines'), size = 3) + labs(title = "All Subjects")
...
ggplot(ggpredict_ordinaldata1, aes(x = Rating, y = predicted)) + 
  geom_point(aes(color = Headlineveracity), position =position_dodge(width = 0.5)) + geom_errorbar(aes(ymin = conf.low, ymax = conf.high, color = Headlineveracity), position = position_dodge(width = 0.5), width = 0.3) + 
  theme_minimal() + facet_wrap(~group) + xlab("Likelihood of Sharing") + ylab("Predicted Probability of Sharing") + geom_label_repel(aes(label=sprintf("%.3f", predicted),fill = Headlineveracity), box.padding = 0.5, min.segment.length = unit(0, 'lines'), size = 3) + labs(title = "\u2265 1 Screener Correct")
...
ggplot(ggpredict_ordinaldata2, aes(x = Rating, y = predicted)) + 
  geom_point(aes(color = Headlineveracity), position =position_dodge(width = 0.5)) + geom_errorbar(aes(ymin = conf.low, ymax = conf.high, color = Headlineveracity), position = position_dodge(width = 0.5), width = 0.3) + 
  theme_minimal() + facet_wrap(~group) + xlab("Likelihood of Sharing") + ylab("Predicted Probability of Sharing") + geom_label_repel(aes(label=sprintf("%.3f", predicted),fill = Headlineveracity), box.padding = 0.5, min.segment.length = unit(0, 'lines'), size = 3) + labs(title = "\u2265 2 Screener Correct")
...
ggplot(ggpredict_ordinaldata3, aes(x = Rating, y = predicted)) + 
  geom_point(aes(color = Headlineveracity), position =position_dodge(width = 0.5)) + geom_errorbar(aes(ymin = conf.low, ymax = conf.high, color = Headlineveracity), position = position_dodge(width = 0.5), width = 0.3) + 
  theme_minimal() + facet_wrap(~group) + xlab("Likelihood of Sharing") + ylab("Predicted Probability of Sharing") + geom_label_repel(aes(label=sprintf("%.3f", predicted),fill = Headlineveracity), box.padding = 0.55, min.segment.length = unit(0, 'lines'), size = 2.5) + labs(title = "All 3 Screener Correct")
