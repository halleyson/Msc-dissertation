...
ggplot(ggpredict_ordinaldata, aes(x = Rating, y = predicted)) + geom_point(aes(color = Headlineveracity), position =position_dodge(width = 0.5)) + geom_errorbar(aes(ymin = conf.low, ymax = conf.high, color = Headlineveracity), position = position_dodge(width = 0.5), width = 0.3) + theme_minimal() +facet_wrap(~group)
...
ggplot(p1, aes(x = factor(condition), y = as.numeric(percent), fill = headlineveracity)) + 
  ...
  scale_fill_discrete(name = "Headline Veracity", labels = c("False", "True"))
