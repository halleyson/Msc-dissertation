...
p1 <- aggregate(measurement~condition+newstype, data = pennystudy2plot1, FUN = sum)
...
ggplot(p1, aes(x = condition, y = measurement/428, fill = newstype)) + 
...
ggplot(p2, aes(x = condition, y = measurement/416, fill = newstype)) + 
  geom_bar(stat = 'identity',position = 'dodge') + 
  geom_text(
    aes(label = round(measurement/416, digits = 3)), vjust = 1.5, position = position_dodge(0.9))+
    scale_y_continuous(labels = scales::percent)
```
...
#Find out the sum for each type of news in different condition
p3 <- aggregate(measurement~condition+newstype, data = pennystudy2plot3, FUN = sum)

##double check for correctness
sum(pennystudy2plot3$measurement,na.rm = TRUE)

#since the obs is 1292 for attention > 2, thus 1292/2 = 646 participants. So to get percentage of sharing intention each fake and real news need to divided by 646/2 = 323
ggplot(p3, aes(x = condition, y = measurement/323, fill = newstype)) + 
  geom_bar(stat = 'identity',position = 'dodge') + 
  geom_text(
    aes(label = round(measurement/323, digits = 3)), vjust = 1.5, position = position_dodge(0.9))+
    scale_y_continuous(labels = scales::percent)
```
