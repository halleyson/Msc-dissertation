...
##no idea why it doesn't show conditions (control vs treatment) so I merge original condition to the new one
...
ggplot(ggpredict_ordinaldata3, aes(x = Rating, y = predicted)) + 
  geom_point(aes(color = Headlineveracity), position =position_dodge(width = 0.5)) + geom_errorbar(aes(ymin = conf.low, ymax = conf.high, color = Headlineveracity), position = position_dodge(width = 0.5), width = 0.3) + 
  theme_minimal() + facet_wrap(~group) + xlab("Likelihood of Sharing") + ylab("Predicted Probability of Sharing") + geom_label_repel(aes(label=sprintf("%.3f", predicted),fill = Headlineveracity), box.padding = 0.5, min.segment.length = unit(0, 'lines'), size = 3) + labs(title = "All 3 Screener Correct")
